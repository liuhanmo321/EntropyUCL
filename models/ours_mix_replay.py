...
class Ours_mix_replay(ContinualModel):
    NAME = 'ours_mix_replay'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, len_train_lodaer, transform):
        super(Ours_mix_replay, self).__init__(backbone, loss, args, len_train_lodaer, transform)
        self.buffer = Buffer(self.args.model.buffer_size, self.device)
        self.seen_datasets = 0
    ...
    def cluster(self, train_loader, device):
        ...
        feature_embedded = TSNE(n_components=2, learning_rate='auto', perplexity=40,
                  init='pca').fit_transform(feature_bank.cpu())
        ...
        dbsc = hdbscan.HDBSCAN(metric="euclidean", min_cluster_size=self.args.train.min_cluster_size).fit(feature_embedded)
        ...
        probs = torch.Tensor(cluster_probs)
        _, relatedness_ind = probs.topk(k=self.args.model.buffer_size, dim=0)
        self.buffer.add_data(examples=image_bank[relatedness_ind, :], logits=aug_image_bank[relatedness_ind, :], seen=self.seen_datasets)
        saved_info = pd.Series(label_bank[relatedness_ind]).value_counts()
        print(saved_info)
...
class Buffer:
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, seen=0):
        ...
        # else:
        #     for i in range(example_size):
        #         self.examples[-(i+1)] = examples[i].to(self.device)
        #         if logits is not None:
        #             self.logits[-(i+1)] = logits[i].to(self.device)
