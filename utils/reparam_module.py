...
class ReparamModule(nn.Module):
    ...
    def __init__(self, module):
        ...
        self._param_infos = tuple(param_infos) # name for module and layer stuff 
        self._shared_param_infos = tuple(shared_param_infos) # parameter names that are shared between different modules
        self._param_numels = tuple(param_numels) # number of parameters for each part of module
        self._param_shapes = tuple(param_shapes) # shape of parameters for each part of module
    ...
    def _forward_with_param_and_buffers(self, flat_param, buffers, *inputs, **kwinputs):
        with self.unflattened_param(flat_param):
            with self.replaced_buffers(buffers):
                z = self.module.encoder(*inputs, **kwinputs)
                p = self.module.predictor(z)
                return p
