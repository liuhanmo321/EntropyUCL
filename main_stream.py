...
def main(device, args):
    ...
    for t in range(dataset.N_TASKS):
      ...
      for idx, ((images1, images2, notaug_images), labels) in enumerate(local_progress):
        ...
        idx_per_task = args.train.num_epochs * args.dataset.iter_per_epoch / 5
        if idx % (args.dataset.iter_per_epoch * args.train.knn_interval) == 0:
          learnt_data_set = min(int(idx / idx_per_task) + 1, 5)
          for i in range(learnt_data_set):
        ...
        epoch_dict = {"epoch":idx, "accuracy": mean_acc}
        ...
        with open(result_path + '/' + args.model.cl_model + '_stream.txt', 'a+') as f:
